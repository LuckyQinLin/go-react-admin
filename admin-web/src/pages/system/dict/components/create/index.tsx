import {Button, Drawer, Form, Input, message, Radio, Space, Spin} from "antd";
import React, {useEffect} from "react";
import {useRequest} from "ahooks";
import {DictCreateDrawerProp, DictCreateFormProp} from "@/pages/system/dict/modules.ts";
import {dictCreate} from "@/api/dict.ts";

const DictCreateDrawer: React.FC<DictCreateDrawerProp> = ({visible, close}) => {

    const [form] = Form.useForm<DictCreateFormProp>();

    const {loading, run} = useRequest(dictCreate, {
        manual: true,
        onSuccess: (_)=> {
            message.success('创建字典成功');
            close(true);
        }
    })

    const submitForm = () => form.validateFields().then(value => {
        run(value)
    });

    useEffect(() => {
        if (visible) {
            form.setFieldsValue({status: 1})
        }
    }, [visible])

    return <Drawer
        width={500}
        title="创建字典"
        placement="right"
        onClose={() => close(false)}
        open={visible}
        extra={
            <Space>
                <Button type="primary" danger onClick={()=> close(false)}>取消</Button>
                <Button type="primary" onClick={submitForm}>保存</Button>
            </Space>
        }
    >
        <Spin tip="加载中......" spinning={loading}>
            <Form labelCol={{ span: 5 }}
                  wrapperCol={{ span: 19 }}
                  form={form}
                  layout="horizontal"
                  name="form_in_modal"
            >
                <Form.Item name="dictName" label="字典名称" rules={[{ required: true, message: '请输入字典名称' }]}>
                    <Input placeholder="请输入字典名称" />
                </Form.Item>
                <Form.Item name="dictType" label="字典类型" rules={[{ required: true, message: '请输入字典类型' }]}>
                    <Input placeholder="请输入字典类型" />
                </Form.Item>
                <Form.Item name="status" label="字典状态">
                    <Radio.Group>
                        <Radio value={1}>正常</Radio>
                        <Radio value={0}>停用</Radio>
                    </Radio.Group>
                </Form.Item>
                <Form.Item name="remark" label="备注">
                    <Input.TextArea placeholder="请输入备注" />
                </Form.Item>
            </Form>
        </Spin>
    </Drawer>
}

export default DictCreateDrawer;